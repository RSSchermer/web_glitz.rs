...
use crate::pipeline::interface_block::Compatibility;
...
unsafe impl IntefaceBlockComponent for Test0 {
    fn check_compatibility<I>(base_offset: usize, remainder: &mut I) -> CheckCompatibility where I: Iterator<Item=&MemoryUnitDescriptor> {
        let check_member0 = float::check_compatibility(base_offset + offset_of!(Test0, member0), remainder);

        if check_member0 != CheckCompatibility::Continue {
            return check_member0;
        }

        let check_member1 = vec2::check_compatibility(base_offset + offset_of!(Test0, member1), remainder);
...
unsafe impl IntefaceBlockComponent for Test1 {
    fn check_compatibility<I>(base_offset: usize, remainder: &mut I) -> CheckCompatibility where I: Iterator<Item=&MemoryUnitDescriptor> {
        let check_member0 = Test0::check_compatibility(base_offset + offset_of!(Test0, member0), remainder);

        if check_member0 != CheckCompatibility::Continue {
            return check_member0;
        }

        let check_member1 = vec4::check_compatibility(base_offset + offset_of!(Test0, member1), remainder);
...
#[repr(C, align(16))]
struct ArrayElementWrapper<T> where T: Std140ArrayElement {
    element: T
}
            ...
            fn check_compatibility<I>(component_offset: usize, remainder: &mut I) -> CheckCompatibility where I: Iterator<Item=&MemoryUnitDescriptor> {
                if let Some(unit) = remainder.next() {
                    if unit.offset() != component_offset {
                        ...
                        CheckCompatibility::Incompatible(Incompatible::UnitLayoutMismatch(unit.clone(), $layout))
                    } else {
                        CheckCompatibility::Continue
                    }
                } else {
...
impl_interface_block_component!(bvec2, UnitLayout::BoolVector3);
...
impl_interface_block_component!(bvec2, UnitLayout::BoolVector4);
