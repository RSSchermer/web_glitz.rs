use super::{
    AndThen, Join, Join3, Join4, Join5, Map, MapErr, OrElse, Sequence, Sequence3, Sequence4,
    Sequence5, Then,
};
    ...
    T: GpuTask<Ec>,
{
    ...
    fn map_err<B, F>(self, f: F) -> MapErr<T, F, Ec>
    where
        F: FnOnce(T::Error) -> B,
        B: 'static;
    ...
    where
        B: GpuTask<Ec>,
        F: FnOnce(Result<T::Output, T::Error>) -> B;

    fn and_then<B, F>(self, f: F) -> AndThen<T, B, F, Ec>
    where
        B: GpuTask<Ec, Error = T::Error>,
        F: FnOnce(T::Output) -> B;

    fn or_else<B, F>(self, f: F) -> OrElse<T, B, F, Ec>
    where
        B: GpuTask<Ec, Output = T::Output>,
        F: FnOnce(T::Error) -> B;
    ...
    where
        B: GpuTask<Ec, Error = T::Error>,
        C: GpuTask<Ec, Error = T::Error>;

    fn join4<B, C, D>(self, b: B, c: C, d: D) -> Join4<T, B, C, D, Ec>
    where
        B: GpuTask<Ec, Error = T::Error>,
        C: GpuTask<Ec, Error = T::Error>,
        D: GpuTask<Ec, Error = T::Error>;
...
impl<T, Ec> GpuTaskExt<T, Ec> for T
where
    T: GpuTask<Ec>,
{
    fn map<B, F>(self, f: F) -> Map<T, F, Ec>
