use super::{
    AndThen, GpuTask, MapErr, MapOk, OrElse, Progress, TryJoin, TryJoin3, TryJoin4, TryJoin5,
    TrySequence, TrySequence3, TrySequence4, TrySequence5,
};

pub type TryProgress<O, E> = Progress<Result<O, E>>;
...
pub trait TryGpuTaskExt<Ec>: TryGpuTask<Ec> {
    ...
    fn try_sequence5<B, C, D, E>(
        self,
        b: B,
        c: C,
        d: D,
        e: E,
    ) -> TrySequence5<Self, B, C, D, E, Ec>
    where
        B: TryGpuTask<Ec, Error = Self::Error>,
        C: TryGpuTask<Ec, Error = Self::Error>,
        D: TryGpuTask<Ec, Error = Self::Error>,
...
impl<T, Ec> TryGpuTaskExt<Ec> for T
where
    T: TryGpuTask<Ec>,
{
    fn map_ok<B, F>(self, f: F) -> MapOk<Self, F, Ec>
