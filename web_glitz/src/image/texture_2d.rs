    ...
    F: TextureFormat + 'static,
{
    ...
    /// The width of the [Texture2D] in pixels.
    pub width: u32,

    /// The height of the [Texture2D] in pixels.
    pub height: u32,
...
/// Storage for a (partial or complete) mipmap chain for a single 2-dimensional image.
///
/// A [Texture2D] has a width and a height, but no depth. A [Texture2D] defines a [TextureFormat]
/// `F`: all image data stored in the texture is stored in this format.
///
/// The GPU may access the data in a [Texture2D], typically through a [Sampler] or [ShadowSampler],
/// see [Texture2D::sampled_float], [Texture2D::sampled_integer],
/// [Texture2D::sampled_unsigned_integer] and [Texture2D::sampled_shadow]. A sampled [Texture2D]
/// may be bound to a pipeline as a resource, see [web_glitz::pipeline::resources::Resources].
///
...
/// Starting at the base image, which has a predefined width and height, each subsequent image is
/// half the width and half the height height of the previous image, rounded down. A complete mipmap
/// chain ends, when either the width or height has reached `1`. Images in a mipmap chain are also
/// referred to as the "levels" of a mipmap chain and they are numbered, starting at `0`. For
/// example, a base image size `140x80` (width is `140`, height is `80`) has the following mipmap
/// chain: `140x80` (level 0), `70x40` (level 1), `35x20` (level 2), `17x10` (level 3), `8x5` (level
/// 4), `4x2` (level 5), `2x1` (level 6).
///
...
/// [MipmapLevels] for details). However, it most store at least the first level: level `0`, the
    ...
    F: TextureFormat + 'static,
{
    ...
    /// The width of this [Texture2D] in pixels.
    pub fn width(&self) -> u32 {
        self.data.width
    }

    /// The height of this [Texture2D] in pixels.
    ...
    F: TextureFormat,
{
    ...
    /// use web_glitz::image::MipmapLevels;
    ///
    /// let texture = context.create_texture_2d(256, 256, MipmapLevels::Manual(3)).unwrap();
    ...
    F: TextureFormat,
{
    ...
    /// Returns a command which, when executed, replaces the pixel data in this [Level]'s image
    /// with the pixel data provided in `data`.
    ///
    /// The pixel data must be stored in a [ClientFormat] that is suitable for the texture's
    /// [TextureFormat].
    ///
    ...
    F: TextureFormat,
{
    ...
    /// Returns the width of this [LevelSubImage] in pixels.
    pub fn width(&self) -> u32 {
        region_2d_overlap_width(self.handle.data.width, self.level, &self.region)
    }

    /// Returns the height of this [LevelSubImage] in pixels.
    pub fn height(&self) -> u32 {
    ...
    /// Returns a command which, when executed, replaces the pixel data in this [LevelSubImage]'s
    /// image region with the pixel data provided in `data`.
    ///
    /// The pixel data must be stored in a [ClientFormat] that is suitable for the texture's
    /// [TextureFormat].
    ///
    ...
    F: TextureFormat,
{
    ...
    /// use web_glitz::image::MipmapLevels;
    ///
    /// let mut texture = context.create_texture_2d(256, 256, MipmapLevels::Manual(3)).unwrap();
