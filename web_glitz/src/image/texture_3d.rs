...
use crate::image::format::{
    ClientFormat, Filterable, FloatSamplable, IntegerSamplable, TextureFormat,
...
///
/// See [RenderingContext::create_texture_3d] for details.
pub struct Texture3DDescriptor<F>
where
    F: TextureFormat + 'static,
{
    /// The format type the [Texture3D] will use to store its image data.
    ///
    /// Must implement [TextureFormat].
    pub format: F,

    /// The width of the [Texture3D].
    pub width: u32,

    /// The height of the [Texture3D].
    pub height: u32,

    /// The depth of the [Texture3D].
    pub depth: u32,

    /// The number of levels in the mipmap for the [Texture3D].
    ///
...
/// # Mipmap
///
/// A [Texture3D] stores a partial or complete mipmap chains its layered base image (or, if you
...
/// levels with [Texture3D::generate_mipmap] (see [Texture3D::generate_mipmap] for details).
///
/// # Sampling
///
/// The GPU may access the data in a [Texture3D], typically through a [Sampler] or [ShadowSampler],
/// see [Texture3D::sampled_float], [Texture3D::sampled_integer],
/// [Texture3D::sampled_unsigned_integer] and [Texture3D::sampled_shadow]. A sampled [Texture3D] may
/// be bound to a pipeline as a resource, see [web_glitz::pipeline::resources::Resources].
///
/// # Example
///
    ...
    F: TextureFormat + 'static,
{
    ...
    /// # use web_glitz::image::texture_3d::Texture3DDescriptor;
    /// # fn wrapper<Rc>(context: &Rc) where Rc: RenderingContext + Clone + 'static {
    /// # let texture = context.create_texture_2d(&Texture3DDescriptor {
    /// #     format: RGB8,
    /// #     width: 256,
    /// #     height: 256,
    /// #     depth: 16,
    ...
    F: TextureFormat + Filterable + 'static,
{
    ...
    /// allocated when the texture was created (see [RenderingContext::create_texture_3d]) has
    ...
    F: TextureFormat + FloatSamplable + 'static,
{
    ...
    pub fn float_sampled(
        &self,
        sampler: &Sampler,
    ) -> Result<FloatSampledTexture3D, IncompatibleSampler> {
    ...
    F: TextureFormat + IntegerSamplable + 'static,
{
    ...
    pub fn integer_sampled(
        &self,
        sampler: &Sampler,
    ) -> Result<IntegerSampledTexture3D, IncompatibleSampler> {
        if self.data().context_id() != sampler.data().context_id() {
            panic!("Texture and sampler do not belong to the same context.");
        }
    ...
    F: TextureFormat + UnsignedIntegerSamplable + 'static,
{
    ...
    pub fn unsigned_integer_sampled(
        &self,
        sampler: &Sampler,
    ) -> Result<UnsignedIntegerSampledTexture3D, IncompatibleSampler> {
        if self.data().context_id() != sampler.data().context_id() {
            panic!("Texture and sampler do not belong to the same context.");
        }
...
where
    F: TextureFormat,
{
    /// The number of levels defined for the [Texture3D].
...
/// A reference to a mipmap level of a [Texture3D].
///
    ...
    F: TextureFormat,
{
    ...
    ///
    /// Note that the depth is the same for all levels of a [Texture3D].
    pub fn depth(&self) -> u32 {
...
/// Returned from [Texture3D::levels_mut], a mutable reference to the levels of a [Texture3D].
///
/// See [Texture3D::levels_mut] for details.
///
///
/// [Deref]s to [Levels].
...
/// Returned from [Texture3D::generate_mipmap_command], generates the image data for a [Texture3D]'s
/// mipmap chain.
///
/// See [Texture3D::generate_mipmap_command] for details.
